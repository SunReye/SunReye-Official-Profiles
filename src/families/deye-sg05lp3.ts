import { control, defineFamily, metric, sumOf } from "@sunreye/profile-sdk";
import type { MetricDataDef, ModelOverrides } from "@sunreye/profile-sdk";
...
/**
 * Deye SG05LP3 hybrid inverter family (the Sunsynk badge of the same hardware
 * shares this register layout). `defineFamily` emits the generic base profile
 * (`deye-sg05lp3`) followed by one self-contained profile per SKU — each a keyed
 * overlay tightening the battery current ceiling. Data profiles carry no
 * `simulate` hook; SunReye's generic role-based simulator drives the demo.
 */
